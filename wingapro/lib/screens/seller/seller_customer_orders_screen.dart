...
class _SellerCustomerOrdersScreenState extends State<SellerCustomerOrdersScreen> {
  ...
  Future<void> _loadOrders() async {
    ...
    try {
      final orders = await SellerSimulationService.getCustomerOrders();
      // Ensure only paid orders are shown (already filtered by service)
      setState(() {
        _orders = orders;
        _loading = false;
      });
  ...
  Future<void> _updateDeliveryStatus(Map<String, dynamic> order, String newStatus) async {
    ...
    final success = await SellerSimulationService.updateDeliveryStatus(order['id'], newStatus, rejectionReason: rejectionReason);
    if (success) {
                  ...
                  Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        // Action buttons (only if not already Delivered or Rejected)
